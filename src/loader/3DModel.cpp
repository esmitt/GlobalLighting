...
void C3DModel::fillNode(const aiScene* scene, const struct aiNode* pNode, glm::mat4 mMatrix, int & iOffset, std::vector<Vertex> & vVertex, std::vector<unsigned int> & vIndex)
{
...
{
	std::cout << "loading the file " << sFilename << std::endl;
	//aiScene* scene;
...
void C3DModel::loadTextures(const std::string& sFilename, const aiScene* pScene)
{
	...
	for (int i = 0 ; i < pScene->mNumMaterials ; i++)
	{
		...
		if (!m_vTexture[i].isValid() && !repeated)
		{
			...
			m_vTexture[i].loadTexture("assets/white.png");
			//pMaterial->
